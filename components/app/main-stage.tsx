...
import { VideoCanvasStage, type VideoCanvasStageRef } from "../../src/components/app/video-canvas-stage";
  ...
  return (
    <div className="flex-1 flex flex-col bg-toolbar p-4">
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2">
          ...
          <Button 
            ...
            onClick={videoCanvasRef.current?.isPaused ? handleResume : handleAnimate}
            disabled={!currentChord || !showVideoCanvas || (videoCanvasRef.current?.isAnimating && !videoCanvasRef.current?.isPaused)}
          >
            <Play />
          </Button>
          <Button 
      ...
      <div className="flex-1 bg-black rounded-lg flex items-center justify-center p-4">
        {showVideoCanvas && selectedChords.length > 0 ? (
          <VideoCanvasStage 
            ref={videoCanvasRef} 
          ...
          <ChordDiagram chord={currentChord} scale={1.5} />
        ) : (
          <>
            <p className="text-muted-foreground">Select a chord from the library to get started</p>
            {/* Componente oculto para carregar FFmpeg em background */}
