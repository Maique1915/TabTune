...
export const FretboardStage = React.forwardRef<FretboardStageRef, FretboardStageProps>(({
    ...
    const drawFrame = useCallback((state: AnimationState, timeMs: number) => {
        ...
        if (ctx) {
            ...
            if (animationType === "static-fingers") {
                ...
                ctx.fillStyle = colors.cardColor || '#000000';
        ...
        if (animationType === "guitar-fretboard" && guitarFretboardDrawerRef.current) {
            ...
            if (chords && chords.length > 0) {
                ...
                if (currentChordData) {
                    // Transition not supported for guitar-fretboard drawer yet, draw static
        ...
        if (timelineState && scoreDrawerRef.current) {
            timelineState.tracks.forEach(track => {
                ...
                const clip = track.clips.find((c: any) => timeMs >= c.start && timeMs < c.start + (c.duration || 0));
                if (clip && clip.type === 'symbol') {
                    scoreDrawerRef.current?.draw(width, height, clip);
                }
            });
        }
    }, [chords, height, timelineState, width, animationType, previewChord, showChordName, capo, buildEnabled, colors, transitionsEnabled]);
    ...
    const computeStateAtTimeMs = useCallback((timeMs: number) => {
        ...
        for (let i = 0; i < chords.length; i++) {
            ...
            if ((animationType === "static-fingers" || animationType === "guitar-fretboard" || animationType === "carousel") && transitionsEnabled) {
                ...
                const currentSegmentTransitionIn = (i > 0) ? Math.min(halfTransitionMsBase, segmentMs) : 0; // Use halfTransitionMsBase instead of segmentMs / 2. This is different
