...
export const ToolsPanel: React.FC<SidebarProps> = ({
    ...
    if (!activeMeasure) {
        return (
            <EmptyState
                icon={Wrench}
                title={t('editor.tools.note_tools')}
                description={t('editor.tools.desc')}
                features={[
                    t('editor.tools.selective'),
                    t('editor.tools.semitone'),
                    t('editor.tools.global')
                ]}
            />
        );
    }
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right-2 duration-300 h-full flex flex-col">
            <div className="space-y-4">
                <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{editingNote ? t('editor.tools.note_tools') : t('editor.tools.measure_tools')}</h3>
                ...
                <div className="bg-zinc-950/40 rounded-3xl p-5 border border-white/[0.02] shadow-xl">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] leading-tight">
                                {editingNote ? t('editor.tools.selective') : t('editor.tools.measure')}
                            </span>
                            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] leading-tight">
                                {t('editor.tools.transpose')}
                            </span>
                        </div>
                        <div
                            onClick={() => {
                        ...
                        >
                            <div className="flex flex-col items-center">
                                <span className="text-[8px] font-black leading-none">{t('editor.tools.auto')}</span>
                                <span className="text-[8px] font-black leading-none mt-0.5">{t('editor.tools.finger')}</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center justify-center gap-4">
                        ...
                        <div className="flex flex-col items-center justify-center min-w-[80px] h-14 bg-black/60 rounded-2xl border border-white/[0.05] shadow-inner">
                            ...
                            <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mt-1">{t('editor.tools.semitone')}</span>
                        </div>
